class CollectionsController < ApplicationController
  ...
  def index
    ...
    if !params[:search].nil?
      @collections_total_number = Collection.count(:all, :conditions => "status = 1 AND title LIKE '%#{params[:search]}%'")
    else
      @collections_total_number = Collection.count(:all, :conditions => "status = 1")
    end
