    ...
    if (!(log_activities.nil?))
      log_activities.each do |activity|
        if(activity[:table_type] == "collection")
          ...
          log_record = "<a class='author' href = '/users/#{collection.user_id}'>#{User.find(collection.user_id).real_name}</a> #{I18n.t(:log_activity_create)} <a class= 'book_title' href = '/collections/#{collection.id}'>#{collection.title}</a> #{ I18n.t(:log_activity_at)} #{activity[:time]}"
          ...
          log_record = "<a class='author' href = '/users/#{collection_rating.user_id}'>#{User.find(collection_rating.user_id).real_name}</a> #{I18n.t(:log_activity_rate)} <a class= 'book_title' href = '/collections/#{collection_rating.collection_id}'>#{Collection.find(collection_rating.collection_id).title}</a> #{I18n.t(:log_activity_with)} #{collection_rating.rate} #{ I18n.t(:log_activity_at)} #{activity[:time]}"
          ...
          log_record = "<a class='author' href = '/users/#{comment.user_id}'>#{User.find(comment.user_id).real_name}</a> #{I18n.t(:log_activity_comment)}  <a class= 'book_title' href = '/books/#{(Volume.find(comment.volume_id)).job_id}'>#{Volume.find(comment.volume_id).book.title}</a>: #{comment.text} #{ I18n.t(:log_activity_at)} #{activity[:time]}" if !(comment.volume_id.nil?)
          # log record of each comment on a collection contains user name , collection name and comment text
          log_record = "<a class='author' href = '/users/#{comment.user_id}'>#{User.find(comment.user_id).real_name}</a> #{I18n.t(:log_activity_comment)}  <a class= 'book_title' href = '/collections/#{comment.collection_id}'>#{Collection.find(comment.collection_id).title}</a>: #{comment.text} #{ I18n.t(:log_activity_at)} #{activity[:time]}" if !(comment.collection_id.nil?)
          # log record of each reply on a comment contains user name , base comment text and reply text
          log_record = "<a class='author' href = '/user/#{comment.user_id}'>#{User.find(comment.user_id).real_name}</a> #{I18n.t(:log_activity_comment)} #{Comment.find(comment.comment_id).text}: #{comment.text} #{ I18n.t(:log_activity_at)} #{activity[:time]}" if !(comment.comment_id.nil?)
