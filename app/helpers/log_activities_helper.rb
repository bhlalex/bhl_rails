    ...
    if (!(log_activities.nil?))
      log_activities.each do |activity|
        if(activity[:table_type] == "collection")
          ...
          log_record = "<a href = '/users/show/#{collection.user_id}'>#{User.find(collection.user_id).real_name}</a> #{I18n.t(:log_activity_create)} <a href = '/collections/list_books_in_collection/#{collection.id}'>#{collection.title}</a>"
          ...
          log_record = "<a href = '/users/show/#{book_rating.user_id}'>#{User.find(book_rating.user_id).real_name}</a> #{I18n.t(:log_activity_rate)} <a href = '/books/#{(Volume.find(book_rating.volume_id)).job_id}'> #{Volume.find(book_rating.volume_id).book.title}</a> #{I18n.t(:log_activity_with)} #{book_rating.rate}"
          ...
          log_record = "<a href = '/users/show/#{collection_rating.user_id}'>#{User.find(collection_rating.user_id).real_name}</a> #{I18n.t(:log_activity_rate)} <a href = '/collections/list_books_in_collection/#{collection_rating.collection_id}'>#{Collection.find(collection_rating.collection_id).title}</a> #{I18n.t(:log_activity_with)} #{collection_rating.rate}"
        else
          comment = Comment.find(activity[:id])
          # log record of each comment on a book contains user name , book name and comment text
          log_record = "<a href = '/users/show/#{comment.user_id}'>#{User.find(comment.user_id).real_name}</a> #{I18n.t(:log_activity_comment)}  <a href = '/books/#{(Volume.find(comment.volume_id)).job_id}'>#{Volume.find(comment.volume_id).book.title}</a>: #{comment.text}" if !(comment.volume_id.nil?)
            # log record of each comment on a collection contains user name , collection name and comment text
          log_record = "<a href = '/users/show/#{comment.user_id}'>#{User.find(comment.user_id).real_name}</a> #{I18n.t(:log_activity_comment)} <a href = '/collections/list_books_in_collection/#{comment.collection_id}'>#{Collection.find(comment.collection_id).title}</a>: #{comment.text}" if !(comment.collection_id.nil?)
            # log record of each reply on a comment contains user name , base comment text and reply text
          log_record = "<a href = '/users/show/#{comment.user_id}'>#{User.find(comment.user_id).real_name}</a> #{I18n.t(:log_activity_comment)} #{Comment.find(comment.comment_id).text}: #{comment.text}" if !(comment.comment_id.nil?)
        end
