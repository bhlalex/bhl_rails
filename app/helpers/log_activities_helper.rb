    ...
    if (!(log_activities.nil?))
      log_activities.each do |activity|
        if(activity[:table_type] == "collection")
          ...
          log_record = "<h3><a href = '/users/show/#{comment.user_id}'>#{User.find(comment.user_id).real_name}</a> #{I18n.t(:log_activity_comment)}  <a href = '/collections/show/#{comment.collection_id}'>#{Collection.find(comment.collection_id).title}</a>: #{comment.text}</h3> <h5> #{activity[:time]}</h5>" if !(comment.collection_id.nil?)
